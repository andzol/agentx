...
#   - Task YAMLs are read live from the public GitHub repo's tasks/ folder
#     on every run (see GITHUB_TASKS_REPO below) - no bucket needed. Add a
#     new task by pushing a new tasks/*.yaml file to the repo.
...
GITHUB_TASKS_REPO="${GITHUB_TASKS_REPO:-andzol/agentx}"
...
gcloud run jobs deploy "${JOB_NAME}" \
  --image="${IMAGE}" \
  --region="${REGION}" \
  --set-env-vars="GITHUB_TASKS_REPO=${GITHUB_TASKS_REPO},SHEETS_SA_KEY_PATH=/secrets/sheets-sa-key.json" \
